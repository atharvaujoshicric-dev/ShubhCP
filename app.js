...
(function () {
  ...
  var SLAB1_THRESHOLD = 35; // Cr
  var SLAB2_THRESHOLD = 50; // Cr
  var SLAB1_RATE = 0.04;
  var SLAB2_RATE = 0.05;
  ...
  function seedData() {
    return [
      { id: 'L1001', name: 'Aarav Mehta', phone: '9820012345', config: '4 BHK Skyvilla', cpName: 'Prestige Realty Partners', visitDate: '2026-05-12T11:00', status: 'Booked', saleValue: 52 },
      { id: 'L1002', name: 'Ishita Rao', phone: '9876543210', config: '3 BHK Skyvilla', cpName: 'Elite Property Consultants', visitDate: '2026-05-18T16:30', status: 'Booked', saleValue: 38 },
  ...
  function getSlab(totalSales) {
    if (totalSales >= SLAB2_THRESHOLD) return { tier: 2, rate: SLAB2_RATE, label: '5% Slab' };
    if (totalSales >= SLAB1_THRESHOLD) return { tier: 1, rate: SLAB1_RATE, label: '4% Slab' };
  ...
  function getCPStats() {
    ...
    return Object.keys(map).map(function (k) {
      var cp = map[k];
      var slab = getSlab(cp.totalSales);
      cp.slab = slab;
      cp.commission = +(cp.totalSales * slab.rate).toFixed(2);
      return cp;
  ...
  function renderLeadTable() {
    ...
    leads.slice().reverse().forEach(function (lead) {
      ...
      tr.innerHTML =
        ...
        '<td data-label="Sourcing CP">' + escapeHtml(lead.cpName) + '</td>' +
  ...
  function renderStats() {
    ...
    var totalSales = bookings.reduce(function (s, l) { return s + (Number(l.saleValue) || 0); }, 0);
    var cpStats = getCPStats();
    var totalCommission = cpStats.reduce(function (s, c) { return s + c.commission; }, 0);
  ...
  function renderLeaderboard() {
    ...
    if (cpStats.length === 0) {
      leaderboardGrid.innerHTML = '<p class="empty-state">No channel partners tracked yet.</p>';
      return;
    }

    cpStats.forEach(function (cp) {
      var scaleMax = Math.max(SLAB2_THRESHOLD * 1.2, cp.totalSales * 1.05, 1);
      var fillPct = Math.min(100, (cp.totalSales / scaleMax) * 100);
      var tick1Pct = (SLAB1_THRESHOLD / scaleMax) * 100;
      var tick2Pct = (SLAB2_THRESHOLD / scaleMax) * 100;
      ...
      card.innerHTML =
        '<div class="cp-card-head">' +
          ...
          '<span class="slab-badge slab-badge--' + cp.slab.tier + '">' + cp.slab.label + '</span>' +
        '</div>' +
        '<div class="slab-meter-wrap">' +
          '<div class="slab-meter">' +
            '<div class="slab-meter-fill" style="width:' + fillPct + '%"></div>' +
            '<div class="slab-meter-tick" style="left:' + tick1Pct + '%" data-label="35 Cr"></div>' +
            '<div class="slab-meter-tick" style="left:' + tick2Pct + '%" data-label="50 Cr"></div>' +
          '</div>' +
        '</div>' +
        '<div class="cp-metrics">' +
          '<div class="cp-metric"><span>Total Sales</span><strong>' + formatCr(cp.totalSales) + '</strong></div>' +
  ...
  leadForm.addEventListener('submit', function (e) {
    ...
    formMsg.textContent = 'Thank you! The site visit has been logged successfully.';
